...
//associate a letter to each column of card
...
{
    int i = 0, j = 0;
    for(i = 0; i < CARD_LINES; i++)
        for(j = 0; j < CARD_COLS; j++)
...
//ASC - sort a column in ascendent order
void asc_sort(cell **card, int column)
{
    int ok = 1, i = 0;
    while(ok)
...
//DSC - sort a column in descendent order
void desc_sort(cell **card, int column)
{
    int ok = 1, i = 0;
    while(ok)
...
//dynamically allocate a 2d matrix for a card
cell** create_matrix(int l, int c)
{
    int i = 0, j = 0;
    ...
    if(matrix)
    {
        for(i = 0; i < l; i++)
        {
            ...
            for(j = 0; j < c; j++) matrix[i][j] = init_cell();
        }
    }
